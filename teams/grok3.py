...
def logic(arena_data: dict):
    ...
    # Hard deploy Wizard if available and suitable
    if "Wizard" in hand and my_elixir >= 5 and not wizard_on_field:
        deploy_position = (random_x(-10, 10), 0)
        deploy_list.list_.append(("Wizard", deploy_position))
        print(f"Force deploying Wizard at position {deploy_position}")
        return
        ...
        for troop in hand:
            ...
            for opp_troop in opp_troops:
                ...
                if can_hit:
                    ...
                    # Cap proximity multiplier at 2x
                    proximity_factor = min(2, 1 / max(0.01, opp_troop.position[1] / 100))
                    score += damage * proximity_factor
            if len(opp_troops) > 1 and "splash" in troops_data[troop]["flags"]:
                score *= 2  # Increased splash bonus
            if score > best_score:
                best_score = score
                best_troop = troop
    else:  # Offensive mode
        if "Wizard" in hand and my_elixir < 5 and not wizard_on_field:
            return  # Delay deployment to save for Wizard
        for troop in hand:
            if troops_data[troop]["cost"] > my_elixir:
                continue
    ...
    # Prevent elixir leakage at 7
    elif my_elixir >= 7 and hand:
        if "Wizard" in hand and not wizard_on_field:
            deploy_position = (random_x(-10, 10), 0)
            deploy_list.list_.append(("Wizard", deploy_position))
            print(f"Deploying Wizard to prevent elixir leakage at position {deploy_position}")
        else:
